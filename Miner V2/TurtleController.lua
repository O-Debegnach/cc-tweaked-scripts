...
function TurtleController:new(config)
...
function TurtleController:refuel()
    if not self.movementConfig.autoRefuel then return true end
...
function TurtleController:move(x, y, z)
    ...
    if (x ~= 0) then
        ...
        if self.rotation ~= rotation then self:setRotation(rotation) end
...
function TurtleController:digForward()
    while turtle.detect() do turtle.dig() end
end

function TurtleController:digUp()
    while turtle.detectUp() do turtle.digUp() end
end

function TurtleController:digDown()
